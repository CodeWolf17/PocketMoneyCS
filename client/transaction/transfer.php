...
<div class="card card-outline card-primary">
    ...
    <div class="card-footer">
        <div class="d-flex w-100">
            <button form="account-form" class="btn btn-default">Cancel</button>
</div>
</div>
...
<script>
    $(function(){
        $('#generate_pass').click(function(){
            ...
            $('[name="confirmed_pin"]').val(randomstring)
        })
        ...
        $.ajax({
            ...
            success: function(resp) {
                if (resp.status == 'success') {
                    ...
                    checks.addClass('text-danger')
                    checks.text('Account doesnt exist')
...
if (pin !== '<?php echo $_settings->userdata('pin'); ?>') {
    ...
    return false;
        ...
        if(parseFloat($('[name="current"]').val()) < parseFloat($('[name="balance"]').val())){
            alert_toast("Amount is greater than the client's balance", 'warning')
            return false;
        }

        if ($('#pin-form').is(':visible')) {
            start_loader()
            if($('.err_msg').length > 0)
                $('.err_msg').remove()
            $.ajax({
